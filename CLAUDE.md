...
- `whatsapp` — calls `whatsapp.send_otp(phone, code)`. `whatsapp.py` has two backends switched by `whatsapp_provider` in `site_config.json`: `meta` (Cloud API with an authentication template) or `vonage` (Messages API / Sandbox — free-form text because the sandbox has no template approval).
- `sms` — calls `sms.send_sms` (Twilio).

Switching providers requires zero code changes — just edit `OTP Settings` and `site_config.json`. In console mode `Phone OTP.code_hash` is blank; in `whatsapp`/`sms` mode `code` is blank and only the hash is stored.
